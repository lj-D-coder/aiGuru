...
export const userProfile = async (req, res, next) => {
  ...
  try {
    ...
    validUser.newUser = false;
      const update = await validUser.save();
      if (!update) { 
...
export const feedback = async (req, res, next) => {
  ...
  try {
    if (!userId || userId.trim() === "" || !feedback || feedback.trim() === "") {
      return res.status(400).json({
        message: "send all required fields: userId, feedback",
      });
    }
